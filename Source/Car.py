...
#Get the video
video = cv2.VideoCapture('VideoSource/Car.mp4')
...
#Create video result
writer = cv2.VideoWriter('Result/Car.mp4', cv2.VideoWriter_fourcc(*'DIVX'), 25, (width, height))
...
while ret == True:
    ...
    for i in np.arange(0, object_detections.shape[2]):
        ...
        if confidence > 0.5:
            ...
            #if not a car delete that frame
